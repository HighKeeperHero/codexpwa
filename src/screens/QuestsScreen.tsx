    ...
    <>
      ...
      <button
        ...
        style={{
          ...
          background: loading ? 'var(--surface)' : 'linear-gradient(135deg, rgba(200,160,78,0.85), rgba(200,94,40,0.6))',
          color: loading ? 'var(--text-3)' : '#0B0A08',
      ...
      <div className="xp-track" style={{ height: 3 }}>
        <div className="xp-fill" style={{
          ...
          background: obj.completed
            ? 'linear-gradient(90deg, #6A8A5A, #8AAA7A)'
...
export function QuestsScreen() {
  ...
  const loadBoard = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Could not load hunts');
    } finally {
      setLoading(false);
    }
  }, [hero?.root_id]);
  ...
  const renderQuest = (quest: Quest) => {
    ...
    const accentColor     = isAlignmentHunt ? ac
      : quest.status === 'completed' ? '#6A8A5A'
      : 'var(--bronze)';
        ...
        <div style={{ display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, padding: '14px 14px' }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap', alignItems: 'center' }}>
              {quest.status === 'active' && (
                <span style={{ fontSize: 8, letterSpacing: '0.12em', color: ac, border: `1px solid ${ac}40`, padding: '2px 7px', borderRadius: 4, fontWeight: 700 }}>ACTIVE</span>
              )}
              {quest.status === 'completed' && (
                <span style={{ fontSize: 8, letterSpacing: '0.12em', color: '#6A8A5A', border: '1px solid rgba(72,110,72,0.4)', padding: '2px 7px', borderRadius: 4, fontWeight: 700 }}>COMPLETE</span>
              )}
              {quest.status === 'locked' && (
                <span style={{ fontSize: 8, color: 'var(--text-3)', border: '1px solid var(--border)', padding: '2px 7px', borderRadius: 4 }}>LOCKED</span>
              )}
              ...
              {quest.rewards?.xp && (
                <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--ember)', fontWeight: 600 }}>+{quest.rewards.xp} XP</span>
              )}
            </div>
            ...
            {isOpen && (
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border)' }} onClick={e => e.stopPropagation()}>
                <p style={{ fontSize: 9, letterSpacing: '0.12em', color: 'var(--text-3)', fontWeight: 600, marginBottom: 10 }}>
    ...
    <div className="screen screen-enter">
      <div className="screen-content stagger">
        ...
        {!loading && isLocked && (
          <div style={{ padding: '20px', borderRadius: 12, background: 'var(--surface)', border: '1px solid var(--border)', textAlign: 'center', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontSize: 11, color: 'var(--ember)', fontWeight: 600, flexShrink: 0 }}>Lv {fateLevel} / 20</span>
            </div>
          </div>
        )}
        ...
        {!loading && !isLocked && !hasAlignment && (
          <div style={{ padding: '20px', borderRadius: 12, marginBottom: 16,
            background: 'linear-gradient(135deg, rgba(200,160,78,0.08), rgba(200,94,40,0.04))',
            border: '1px solid rgba(200,160,78,0.2)', textAlign: 'center',
          }}>
        ...
        {!loading && !error && hasAlignment && alignmentHunts.length > 0 && (
          <>
            <div className="divider">
              <span className="divider-label" style={{ color: ac }}>{meta!.label.toUpperCase()} HUNTS · {alignmentHunts.length}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {alignmentHunts.map(renderQuest)}
            </div>
          </>
        )}

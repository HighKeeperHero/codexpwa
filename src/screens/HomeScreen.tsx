...
// Sprint 10: Live Session Feed — when wearable.status === 'active', shows
// a real-time session banner (venue, duration, party members nearby).
...
// ── Live Session Feed ─────────────────────────────────────────────────────────
// Shown at the top of HomeScreen when wearable.status === 'active'.
// Polls /api/sessions/active/:rootId every 30s while mounted.

function LiveSessionFeed({ rootId, wearable }: { rootId: string; wearable: any }) {
  const [session, setSession]   = useState<LiveSession | null>(null);
  const [elapsed, setElapsed]   = useState('—');
  const [pulse, setPulse]       = useState(false);
  const acColor = '#34d399';  // green pulse for active
  ...
  // Tick elapsed timer every minute
  useEffect(() => {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ borderRadius: 14, overflow: 'hidden',
        ...
        background: `linear-gradient(135deg, var(--surface), ${acColor}0C)`,
        boxShadow: `0 0 24px ${acColor}15` }}>
        {/* Live pulse bar */}
        <div style={{ height: 2, background: `linear-gradient(90deg, ${acColor}, ${acColor}40)`,
        ...
        <div style={{ padding: '13px 15px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: acColor,
              boxShadow: `0 0 8px ${acColor}`, animation: 'livePulse 2s ease-in-out infinite',
              flexShrink: 0 }} />
            ...
            <span style={{ fontFamily: 'Cinzel, serif', fontSize: 9, fontWeight: 700,
              color: acColor, letterSpacing: '0.2em', textTransform: 'uppercase' }}>
              Session Active
            </span>
          ...
          <p style={{ fontFamily: 'Cinzel, serif', fontSize: 16, fontWeight: 700,
            color: 'var(--text-1)', margin: '0 0 4px', letterSpacing: '0.04em' }}>
            {venue}
          </p>
          ...
          {session ? (
            <>
              <p style={{ fontSize: 11, color: 'var(--text-3)', margin: '0 0 10px' }}>
                {session.session_id
                  ? `Session ${session.session_id.slice(-6).toUpperCase()}`
              ...
              {session.party && session.party.length > 0 && (
                <div>
                  <p style={{ fontSize: 9, color: 'var(--text-3)', letterSpacing: '0.15em',
                    textTransform: 'uppercase', marginBottom: 8 }}>
                    Party — {session.party.length} hero{session.party.length !== 1 ? 'es' : ''}
                  </p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {session.party.map((m: PartyMember) => {
                      const mc = ALIGNMENT_COLOR[m.alignment] ?? '#8899AA';
                      return (
                        <div key={m.root_id} style={{ display: 'flex', alignItems: 'center', gap: 5,
                          background: `${mc}10`, border: `1px solid ${mc}30`,
                          borderRadius: 8, padding: '5px 9px' }}>
                          <div style={{ width: 18, height: 18, borderRadius: 4, flexShrink: 0,
                            background: `${mc}25`, border: `1px solid ${mc}50`,
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: 9, color: mc, fontFamily: 'Cinzel, serif', fontWeight: 700 }}>
                            {m.display_name[0].toUpperCase()}
                          </div>
                          <div>
                            <p style={{ fontFamily: 'Cinzel, serif', fontSize: 10, fontWeight: 700,
                              color: 'var(--text-1)', margin: '0 0 1px',
                              maxWidth: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {m.display_name}
                            </p>
                            <p style={{ fontSize: 8, color: mc, margin: 0, letterSpacing: '0.05em' }}>
                              Lv {m.fate_level}
                            </p>
                          </div>
                        </div>
              ...
              {session.party && session.party.length === 0 && (
                <p style={{ fontSize: 11, color: 'var(--text-3)', fontStyle: 'italic' }}>
                  Running solo — no party members yet.
                </p>
              )}
            </>
          ) : (
            // Wristband active but no live session data yet
            <p style={{ fontSize: 11, color: 'var(--text-3)', fontStyle: 'italic' }}>
              Wristband tapped — session initializing…
            </p>
          )}
        </div>

        {/* Refresh hint */}
        <div style={{ padding: '6px 15px 8px', borderTop: '1px solid rgba(255,255,255,0.04)',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.1em' }}>
            LIVE · UPDATES EVERY 30S
          </span>
          <button onClick={fetchSession} style={{ background: 'none', border: 'none',
            color: acColor, fontSize: 10, cursor: 'pointer', fontFamily: 'Cinzel, serif',
            letterSpacing: '0.05em', opacity: 0.7, padding: 0 }}>↻ Sync</button>
        </div>
      </div>
    </div>
  );
...
export function HomeScreen({ onSwitchHero }: { onSwitchHero?: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/training/daily/${rootId}`)
      .then(r => r.json()).then(d => {
        const p = d?.data ?? d;
        if (typeof p?.streak === 'number') setTrainingStreak(p.streak);
      }).catch(() => {});
    fetch(`${BASE}/api/training/pillars/${rootId}`)
      .then(r => r.json()).then(d => {
        const p = d?.data ?? d;
        if (Array.isArray(p)) setPillars(p);
      }).catch(() => {});
    fetch(`${BASE}/api/training/oath/${rootId}`)
      .then(r => r.json()).then(d => setOath(d?.data ?? d ?? null))
      .catch(() => setOath(null));
  }, [rootId]);
  ...
  const topStreak     = pillars.length > 0
    ? pillars.reduce((a, b) => a.streak > b.streak ? a : b) : null;
  ...
  return (
    <div className="screen screen-enter" ref={scrollRef}
      ...
      {pulling && (
        <div className="ptr-indicator">
          <span style={{ fontSize: 12, color: 'var(--gold)',
            opacity: pullDist > 55 ? 1 : 0.4, transition: 'opacity 0.2s' }}>
      ...
      <div className="screen-content stagger">
        {/* ── Live Session Feed (active wristband only) */}
        {isInSession && rootId && (
          <LiveSessionFeed rootId={rootId} wearable={wearable} />
        )}
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 className="serif-bold" style={{ fontSize: 34, color: 'var(--text-1)', letterSpacing: 0.5,
            lineHeight: 1.1, marginBottom: topTitle ? 6 : 16 }}>
            {hero.display_name}
          </h1>
          {topTitle && (
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 22 }}>
            ...
            {sources.length > 0 && (
              <span className="badge">
                {sources.filter((s: any) => s.is_active).length} VENUE{sources.filter((s: any) => s.is_active).length !== 1 ? 'S' : ''}
              </span>
            )}
            {isInSession && (
              <span className="badge" style={{ color: '#34d399', borderColor: 'rgba(52,211,153,0.4)',
                background: 'rgba(52,211,153,0.08)', animation: 'none' }}>
                <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#34d399',
                  display: 'inline-block', boxShadow: '0 0 5px #34d399' }} />
                IN SESSION
              </span>
            )}
            {!isInSession && wearable?.status && (
              <span className="badge" style={{ color: 'var(--gold)', borderColor: 'var(--gold-dim)', background: 'var(--gold-glow)' }}>
                ⌚ WRISTBAND
              </span>
            )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', border: '1px solid var(--border)',
            borderRadius: 'var(--radius)', overflow: 'hidden', background: 'var(--surface)', marginTop: 8 }}>
            {[
              ...
              <div key={i} style={{ padding: '13px 0', display: 'flex', flexDirection: 'column',
                alignItems: 'center', gap: 4,
                borderRight: i < 3 ? '1px solid var(--border)' : 'none' }}>
                <span className="serif-bold" style={{ fontSize: 17, color: s.accent ? 'var(--gold)' : 'var(--text-1)' }}>
                  {s.val}
                </span>
        ...
        {equippedSlots.length > 0 && (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1, borderRadius: 12,
              ...
              {equippedSlots.map(({ slot, item }, i) => {
                ...
                return (
                  <div key={slot} style={{ display: 'flex', alignItems: 'center', background: 'var(--surface)',
                    ...
                    <div style={{ width: 3, alignSelf: 'stretch', flexShrink: 0,
                      background: `linear-gradient(180deg, ${rarityColor}cc, ${rarityColor}44)` }} />
                    <div style={{ width: 40, height: 44, flexShrink: 0, display: 'flex',
                      alignItems: 'center', justifyContent: 'center',
                      fontSize: 14, color: `${rarityColor}80` }}>
                      {SLOT_ICON[slot] ?? '·'}
                    </div>
                    <div style={{ flex: 1, minWidth: 0, padding: '10px 0' }}>
                      <p style={{ fontSize: 13, fontFamily: 'var(--font-serif)', fontWeight: 600,
                        color: 'var(--text-1)', lineHeight: 1.2,
                        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {item.item_name ?? item.name ?? 'Unknown'}
                      </p>
                      <p style={{ fontSize: 9, color: 'var(--text-3)', letterSpacing: '0.1em', marginTop: 2 }}>
                        {slot.toUpperCase()}
                      </p>
                    </div>
                    <div style={{ flexShrink: 0, padding: '0 14px', display: 'flex', alignItems: 'center' }}>
                      <span style={{ fontSize: 8, fontWeight: 700, letterSpacing: '0.12em', color: rarityColor,
                        textTransform: 'uppercase', background: `${rarityColor}12`,
                        border: `1px solid ${rarityColor}30`, borderRadius: 4, padding: '2px 6px' }}>
                        {rarity}
                      </span>
                    </div>
            ...
            <div className="card" style={{ padding: '14px 16px', marginBottom: 4 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                {bestVenue.best_boss_pct >= 100 && (
                  <span style={{ fontSize: 10, color: 'var(--gold)', letterSpacing: 1,
                    border: '1px solid var(--gold-dim)', padding: '3px 8px', borderRadius: 4 }}>VEIL CLEARED</span>
                )}
              </div>
              {bestVenue.best_boss_pct > 0 && (
                <div style={{ marginTop: 10 }}>
                  ...
                  <div className="xp-track" style={{ height: 3 }}>
                    <div className="xp-fill" style={{ width: `${bestVenue.best_boss_pct}%`,
                      background: bestVenue.best_boss_pct >= 100
        ...
        <div style={{ marginTop: 28 }}>
          <button onClick={() => setShowCard(true)} style={{ width: '100%', padding: '16px 0',
        ...
        <div style={{ marginTop: 36, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {onSwitchHero && (
            <button onClick={onSwitchHero} style={{ width: '100%', padding: '13px 0',
              background: 'var(--surface)', color: 'var(--text-2)', border: '1px solid var(--border)',
              borderRadius: 10, fontFamily: 'var(--font-serif)', fontSize: 12,
              cursor: 'pointer', letterSpacing: '0.08em' }}>Switch Hero</button>
          )}
          <button onClick={signOut} style={{ width: '100%', padding: '13px 0', background: 'transparent',
            color: 'var(--text-3)', border: '1px solid var(--border)', borderRadius: 10,
            fontFamily: 'var(--font-serif)', fontSize: 12, cursor: 'pointer',
            letterSpacing: '0.08em', opacity: 0.6 }}>Sign Out</button>
        </div>
      </div>
    ...
    <>
      ...
      {displayStreak > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          ...
          background: 'linear-gradient(135deg, var(--surface), rgba(255,165,0,0.06))',
          ...
          {topStreak && topStreak.longest_streak > 0 && (
            <div style={{ textAlign: 'right' }}>
              ...
              <p style={{ fontSize: 16, fontWeight: 700, color: 'rgba(255,165,0,0.5)', fontFamily: 'var(--font-serif)' }}>
      ...
      {pillars.length > 0 && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)',
          ...
          {pillars.map((p, i) => {
            ...
            return (
              <div key={p.pillar} style={{ padding: '11px 14px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 9, color: cfg.color, letterSpacing: '0.12em', fontWeight: 700, flex: 1 }}>
                    {cfg.label} · Lv {p.level}
                  </span>
                  ...
                  <span style={{ fontSize: 9, color: 'var(--text-3)', fontFamily: 'monospace' }}>
                    {p.xp_to_next.toLocaleString()} to next
                  </span>
                </div>
      ...
      {!activeOath && oathLoaded && (
        <div style={{ padding: '11px 14px', borderRadius: 10,
          background: 'rgba(255,165,0,0.04)', border: '1px solid rgba(255,165,0,0.1)',
...
function OathCard({ oath, remaining }: { oath: Oath; remaining: number }) {
  ...
  return (
    <div style={{ borderRadius: 12, overflow: 'hidden', marginBottom: 4,
      ...
      background: `linear-gradient(135deg, var(--surface) 60%, ${cfg.color}08)` }}>
      ...
      <div style={{ padding: '13px 15px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <span style={{ fontSize: 9, color: cfg.color, letterSpacing: '0.14em', fontWeight: 700 }}>
              ACTIVE OATH · {cfg.label}
            </span>
          </div>
          <span style={{ fontSize: 9, color: 'var(--text-3)', fontFamily: 'monospace' }}>
            {formatDDHH(remaining)} left
          </span>
        </div>
        <p style={{ fontSize: 13, color: 'var(--text-1)', lineHeight: 1.6,
          fontFamily: 'var(--font-serif)', fontStyle: 'italic' }}>"{oath.declaration}"</p>
        <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
          <span style={{ fontSize: 9, color: '#8AB06A', background: 'rgba(106,138,90,0.1)',
            border: '1px solid rgba(106,138,90,0.2)', borderRadius: 6, padding: '3px 8px' }}>
            +200 XP if kept
          </span>
          <span style={{ fontSize: 9, color: 'var(--ember)', background: 'rgba(200,94,40,0.06)',
            border: '1px solid rgba(200,94,40,0.15)', borderRadius: 6, padding: '3px 8px' }}>
            −50 XP debt if broken
          </span>
        </div>
      </div>
    </div>

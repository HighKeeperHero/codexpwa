...
const OATH_PRESETS: Record<string, { easy: string[]; medium: string[]; hard: string[] }> = {
  forge: {
    easy:   ['Walk or move for at least 20 minutes every day this week','Complete at least 3 physical rites this week','Stretch or recover for 10 minutes each day'],
    medium: ['Complete a cardio or strength session at least 4 days this week','Train for a minimum of 30 minutes on 4 separate days','Attempt a physical challenge you have been avoiding'],
    hard:   ['Train every single day this week without missing a day','Complete all 3 daily Forge rites every day this week','Log at least 5 hours of total physical training this week'],
  },
  lore: {
    easy:   ['Read for at least 20 minutes on 3 days this week','Write in a journal at least twice this week','Listen to one educational podcast or lecture this week'],
    medium: ['Read for 30 minutes every day this week','Write every day, even if only a few lines','Learn one new skill or concept and apply it before the week ends'],
    hard:   ['Complete all 3 daily Lore rites every day this week','Read for at least an hour every day this week','Write 1,000 words before the week ends'],
  },
  veil: {
    easy:   ['Meditate or sit in silence for 5 minutes on 3 days this week','Perform one act of service for another person this week','Write 5 things you are grateful for on 3 days this week'],
    medium: ['Meditate for 10 minutes every day this week','Spend time in nature without a screen at least twice this week','Have one meaningful conversation with someone you trust this week'],
    hard:   ['Complete all 3 daily Veil rites every day this week','Disconnect from all screens for 2 hours on at least 4 days','Meditate every day and perform an act of service every day'],
  },
};

const DIFFICULTY_LABEL: Record<string, string> = { easy:'EASY', medium:'MEDIUM', hard:'HARD' };
const DIFFICULTY_COLOR: Record<string, string>  = { easy:'#6A8A5A', medium:'#C8A04E', hard:'#C85E28' };
...
function CountdownBadge({ label, ms, format }: { label: string; ms: number; format: 'hhmm' | 'ddhhmm' }) {
  ...
  return (
    <div style={{
      ...
      background:'rgba(37,32,24,0.8)', border:'1px solid var(--border)',
    }}>
...
export function TrainingScreen() {
  ...
  const rootId    = (hero as any)?.root_id ?? (hero as any)?.id ?? null;
  ...
  useEffect(() => {
    ...
    const init = async () => {
      setLoading(true);
      try { await Promise.all([fetchDaily(), fetchPillars(), fetchOath()]); }
  ...
  const completeRite = async (rite: Rite) => {
    ...
    try {
      ...
      } else { showToast(data.message ?? 'Could not complete rite.'); }
  ...
  const midnightMs = getMidnightUTC();
...
// ── Daily View ────────────────────────────────────────────────────────────────
function DailyView({ rites, completing, onComplete, midnightMs }: {
  rites: Rite[]; completing: string | null;
  onComplete: (r: Rite) => void; midnightMs: number;
}) {
  if (rites.length === 0) return (
    <div style={{ textAlign:'center', padding:'48px 0' }}>
      <p style={{ fontSize:13, color:'var(--text-3)', lineHeight:1.8 }}>
        No rites generated yet.<br /><span style={{ fontSize:11 }}>Try refreshing the page.</span>
      </p>
    </div>
  );
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
      ...
      <div>
        ...
        <div style={{ height:3, background:'var(--border)', borderRadius:2 }}>
          <div style={{
            height:'100%', borderRadius:2, width:`${(completed/3)*100}%`,
            background: allDone ? 'linear-gradient(90deg,#6A8A5A,#8AB06A)' : 'linear-gradient(90deg,var(--ember),var(--gold))',
            transition:'width 0.6s cubic-bezier(0.16,1,0.3,1)',
          }} />
        </div>
        <p style={{ fontSize:9, color:'var(--text-3)', marginTop:5, letterSpacing:'0.04em' }}>
          Rites reset at midnight UTC — {Intl.DateTimeFormat().resolvedOptions().timeZone}
        </p>
      </div>
        ...
        return (
          <div key={rite.rite_id} className={`rite-card ${rite.status}`}
            ...
            <div style={{ display:'flex', gap:12, alignItems:'flex-start' }}>
              <div style={{ width:40, height:40, borderRadius:10, flexShrink:0, background: isDone ? 'rgba(106,138,90,0.15)' : `${p.color}15`, border:`1px solid ${isDone ? 'rgba(106,138,90,0.3)' : `${p.color}30`}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <p style={{ fontSize:11, color:'var(--text-3)', lineHeight:1.5, fontStyle:'italic' }}>"{rite.lore_text}"</p>
              </div>
            </div>
            {!isDone && !completing && (
...
// ── Pillars View ──────────────────────────────────────────────────────────────
function PillarsView({ pillars, alignment, midnightMs }: { pillars: PillarData[]; alignment: string; midnightMs: number }) {
  if (pillars.length === 0) return (
    <div style={{ textAlign:'center', padding:'48px 0' }}>
      <p style={{ fontSize:13, color:'var(--text-3)', lineHeight:1.8 }}>
        No pillar data yet.<br /><span style={{ fontSize:11 }}>Complete a daily rite to begin tracking.</span>
      </p>
    </div>
  );
      ...
      {resonance && (
        <div style={{ padding:'10px 14px', borderRadius:10, background:'rgba(200,160,78,0.06)', border:'1px solid rgba(200,160,78,0.15)', display:'flex', gap:10, alignItems:'center' }}>
          ...
          <p style={{ fontSize:11, color:'var(--text-2)', lineHeight:1.5 }}>
            ...
            {' — '}
            {resonance === 'all' ? 'Complete all 3 daily rites for +50 bonus XP' : `+50 bonus XP on ${PILLAR[resonance]?.label ?? ''} rites`}
          </p>
        </div>
      )}
        ...
        return (
          <div key={p.pillar} style={{ padding:'18px', borderRadius:14, background:'var(--surface)', border:`1px solid ${isResonant ? `${cfg.color}30` : 'var(--border)'}` }}>
            <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
              <div style={{ width:44, height:44, borderRadius:10, flexShrink:0, background:`${cfg.color}12`, border:`1px solid ${cfg.color}30`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20 }}>{cfg.icon}</div>
              <div style={{ flex:1 }}>
...
// ── Oath View ─────────────────────────────────────────────────────────────────
type OathStep   = 'pillar' | 'difficulty' | 'pick';
type Difficulty = 'easy' | 'medium' | 'hard';

function OathView({ oath, rootId, weekEndMs, onUpdate, showToast }: {
  oath: Oath | null | undefined; rootId: string | null;
  weekEndMs: number;
  onUpdate: () => Promise<void>; showToast: (msg: string, xp?: number) => void;
}) {
  ...
  const declare = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { showToast(data.message ?? 'Oath entered into the Codex.'); await onUpdate(); }
      else { showToast(data.message ?? 'Could not declare oath.'); }
  ...
  const resolve = async (status: 'kept' | 'broken') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await onUpdate(); setStep('pillar'); setSelected(null);
      } else { showToast(data.message ?? 'Could not resolve oath.'); }
    } catch { showToast('Connection error.'); }
    finally { setLoading(false); }
  };

  if (oath === undefined) return <p style={{ padding:'48px 0', textAlign:'center', fontSize:12, color:'var(--text-3)' }}>Loading oath…</p>;
  ...
  if (activeOath) {
    ...
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => resolve('kept')} disabled={loading} style={{ flex:1, padding:'15px', borderRadius:10, cursor:'pointer', background:'rgba(106,138,90,0.15)', border:'1px solid rgba(106,138,90,0.35)', color:'#8AB06A', fontSize:12, fontWeight:700, letterSpacing:'0.08em' }}>✓ OATH KEPT</button>
          <button onClick={() => resolve('broken')} disabled={loading} style={{ flex:1, padding:'15px', borderRadius:10, cursor:'pointer', background:'rgba(200,94,40,0.08)', border:'1px solid rgba(200,94,40,0.2)', color:'var(--ember)', fontSize:12, fontWeight:700, letterSpacing:'0.08em' }}>✗ OATH BROKEN</button>
        </div>
      </div>
    );
  }
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {Object.entries(PILLAR).map(([key, cfg]) => (
          <button key={key} onClick={() => { setPillar(key); setStep('difficulty'); }} style={{ width:'100%', padding:'16px 18px', borderRadius:12, cursor:'pointer', textAlign:'left', background:`${cfg.color}08`, border:`1px solid ${cfg.color}35`, display:'flex', alignItems:'center', gap:14, transition:'all 0.15s' }}>
  ...
  if (step === 'difficulty') return (
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <p style={{ fontSize:11, color:'var(--text-3)', lineHeight:1.6 }}>All oaths grant the same XP — difficulty is a personal standard, not a variable prize. Choose honestly.</p>
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {(['easy','medium','hard'] as Difficulty[]).map(d => (
          <button key={d} onClick={() => { setDifficulty(d); setStep('pick'); setSelected(null); }} style={{ width:'100%', padding:'16px 18px', borderRadius:12, cursor:'pointer', textAlign:'left', background:'var(--surface)', border:`1px solid ${DIFFICULTY_COLOR[d]}35`, display:'flex', alignItems:'center', gap:14, transition:'all 0.15s' }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:3 }}>
                ...
                <span style={{ fontSize:10, color:'var(--gold)' }}>+200 XP if kept</span>
              </div>
              <p style={{ fontSize:11, color:'var(--text-3)' }}>
                {d==='easy' ? 'A manageable weekly commitment — build the habit first' : d==='medium' ? 'A real challenge that demands consistent effort' : 'A full-week test of will — for those who mean it'}
              </p>
            </div>
  ...
  if (step === 'pick') {
    ...
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <p style={{ fontSize:9, color:'var(--text-3)', letterSpacing:'0.12em' }}>STEP 3 OF 3 — {cfg.label} · {DIFFICULTY_LABEL[difficulty]}</p>
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {options.map(o => (
            <button key={o} onClick={() => setSelected(selected === o ? null : o)} style={{ width:'100%', padding:'16px', borderRadius:12, cursor:'pointer', textAlign:'left', background: selected===o ? `${cfg.color}12` : 'var(--surface)', border:`1px solid ${selected===o ? `${cfg.color}50` : 'var(--border)'}`, transition:'all 0.15s' }}>
        ...
        <button onClick={declare} disabled={!selected || loading} style={{ width:'100%', padding:'15px', borderRadius:10, cursor: selected ? 'pointer' : 'not-allowed', background: selected ? 'var(--gold)' : 'rgba(200,160,78,0.2)', border:'none', color: selected ? 'var(--bg)' : 'var(--text-3)', fontSize:12, fontWeight:700, letterSpacing:'0.12em', transition:'all 0.2s' }}>
          {loading ? 'RECORDING…' : 'ENTER INTO THE CODEX'}
        </button>
        <p style={{ fontSize:11, color:'var(--text-3)', textAlign:'center', lineHeight:1.5 }}>"Your word is entered. The Veil watches."</p>
      </div>
    );
  }
